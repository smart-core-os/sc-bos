...
func Shift(d time.Duration, segments ...*electricpb.ElectricMode_Segment) []*electricpb.ElectricMode_Segment {
	...
	if d > 0 {
		...
		if first := segments[0]; first.Magnitude == 0 {
			...
			first = proto.Clone(first).(*electricpb.ElectricMode_Segment) // clone so we don't update the original
			first.Length = durationpb.New(first.Length.AsDuration() + d)
			out := make([]*electricpb.ElectricMode_Segment, len(segments))
			out[0] = first
			copy(out[1:], segments[1:])
			return out
		}

		out := make([]*electricpb.ElectricMode_Segment, len(segments)+1)
		out[0] = &electricpb.ElectricMode_Segment{Length: durationpb.New(d)}
		copy(out[1:], segments)
		return out
	}
	...
	for i, segment := range segments {
		...
		if cur+l > d {
			...
			out := make([]*electricpb.ElectricMode_Segment, len(segments)-i)
			out[0] = after
