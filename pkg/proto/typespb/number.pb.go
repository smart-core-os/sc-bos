// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v7.34.0
// source: smartcore/bos/types/v1/number.proto

package typespb

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// Possible behaviours for values that are not allowed by the number
type InvalidNumberBehaviour int32

const (
	// A default behaviour will be applied, typically RESTRICT
	InvalidNumberBehaviour_INVALID_NUMBER_BEHAVIOUR_UNSPECIFIED InvalidNumberBehaviour = 0
	// The value will be restricted to the most appropriate value, typically the closest
	InvalidNumberBehaviour_RESTRICT InvalidNumberBehaviour = 1
	// An error will be raised to alert to the invalid value
	InvalidNumberBehaviour_ERROR InvalidNumberBehaviour = 2
	// Ignore the bounds and apply the value anyway
	InvalidNumberBehaviour_ALLOW InvalidNumberBehaviour = 3
)

// Enum value maps for InvalidNumberBehaviour.
var (
	InvalidNumberBehaviour_name = map[int32]string{
		0: "INVALID_NUMBER_BEHAVIOUR_UNSPECIFIED",
		1: "RESTRICT",
		2: "ERROR",
		3: "ALLOW",
	}
	InvalidNumberBehaviour_value = map[string]int32{
		"INVALID_NUMBER_BEHAVIOUR_UNSPECIFIED": 0,
		"RESTRICT":                             1,
		"ERROR":                                2,
		"ALLOW":                                3,
	}
)

func (x InvalidNumberBehaviour) Enum() *InvalidNumberBehaviour {
	p := new(InvalidNumberBehaviour)
	*p = x
	return p
}

func (x InvalidNumberBehaviour) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (InvalidNumberBehaviour) Descriptor() protoreflect.EnumDescriptor {
	return file_smartcore_bos_types_v1_number_proto_enumTypes[0].Descriptor()
}

func (InvalidNumberBehaviour) Type() protoreflect.EnumType {
	return &file_smartcore_bos_types_v1_number_proto_enumTypes[0]
}

func (x InvalidNumberBehaviour) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use InvalidNumberBehaviour.Descriptor instead.
func (InvalidNumberBehaviour) EnumDescriptor() ([]byte, []int) {
	return file_smartcore_bos_types_v1_number_proto_rawDescGZIP(), []int{0}
}

// Options for how invalid number values will be handled
type NumberCapping struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// If the value is less than the minimum, behave this way. Should default to RESTRICT
	Min InvalidNumberBehaviour `protobuf:"varint,1,opt,name=min,proto3,enum=smartcore.bos.types.v1.InvalidNumberBehaviour" json:"min,omitempty"`
	// If the value does not lie on a step value, behave this way. Should default to ALLOW
	Step InvalidNumberBehaviour `protobuf:"varint,2,opt,name=step,proto3,enum=smartcore.bos.types.v1.InvalidNumberBehaviour" json:"step,omitempty"`
	// If the value is greater than the maximum, behave this way. Should default to RESTRICT
	Max           InvalidNumberBehaviour `protobuf:"varint,3,opt,name=max,proto3,enum=smartcore.bos.types.v1.InvalidNumberBehaviour" json:"max,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *NumberCapping) Reset() {
	*x = NumberCapping{}
	mi := &file_smartcore_bos_types_v1_number_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *NumberCapping) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*NumberCapping) ProtoMessage() {}

func (x *NumberCapping) ProtoReflect() protoreflect.Message {
	mi := &file_smartcore_bos_types_v1_number_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use NumberCapping.ProtoReflect.Descriptor instead.
func (*NumberCapping) Descriptor() ([]byte, []int) {
	return file_smartcore_bos_types_v1_number_proto_rawDescGZIP(), []int{0}
}

func (x *NumberCapping) GetMin() InvalidNumberBehaviour {
	if x != nil {
		return x.Min
	}
	return InvalidNumberBehaviour_INVALID_NUMBER_BEHAVIOUR_UNSPECIFIED
}

func (x *NumberCapping) GetStep() InvalidNumberBehaviour {
	if x != nil {
		return x.Step
	}
	return InvalidNumberBehaviour_INVALID_NUMBER_BEHAVIOUR_UNSPECIFIED
}

func (x *NumberCapping) GetMax() InvalidNumberBehaviour {
	if x != nil {
		return x.Max
	}
	return InvalidNumberBehaviour_INVALID_NUMBER_BEHAVIOUR_UNSPECIFIED
}

// Defines the bounds for a number. Either of the min or max values can be absent which means those aspects are
// unbounded. It makes no sense for both to be unset, if so then this is no longer a bounds but that isn't
// disallowed.
//
// Contrary to most programming practices both min and max are inclusive and denote the max allowed value, just like
// htmls input type=range.
type Int32Bounds struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// the minimum value (inclusive). If absent then there is no minimum, though this is typically Int32.Min
	Min *int32 `protobuf:"varint,1,opt,name=min,proto3,oneof" json:"min,omitempty"`
	// the maximum value (inclusive). If absent then there is no maximum, though this is typically Int32.Max
	Max           *int32 `protobuf:"varint,2,opt,name=max,proto3,oneof" json:"max,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *Int32Bounds) Reset() {
	*x = Int32Bounds{}
	mi := &file_smartcore_bos_types_v1_number_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Int32Bounds) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Int32Bounds) ProtoMessage() {}

func (x *Int32Bounds) ProtoReflect() protoreflect.Message {
	mi := &file_smartcore_bos_types_v1_number_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Int32Bounds.ProtoReflect.Descriptor instead.
func (*Int32Bounds) Descriptor() ([]byte, []int) {
	return file_smartcore_bos_types_v1_number_proto_rawDescGZIP(), []int{1}
}

func (x *Int32Bounds) GetMin() int32 {
	if x != nil && x.Min != nil {
		return *x.Min
	}
	return 0
}

func (x *Int32Bounds) GetMax() int32 {
	if x != nil && x.Max != nil {
		return *x.Max
	}
	return 0
}

// Describes features and restrictions applied to a number that is typically used as part of an apis value
type Int32Attributes struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Bounds *Int32Bounds           `protobuf:"bytes,1,opt,name=bounds,proto3" json:"bounds,omitempty"`
	// The stepping interval, how little the value can change in one go. A default value of 0 is equivalent to a
	// continuous step, i.e. step=1
	Step int32 `protobuf:"varint,3,opt,name=step,proto3" json:"step,omitempty"`
	// Indicate whether the device supports delta adjustments when changing the value
	SupportsDelta bool `protobuf:"varint,4,opt,name=supports_delta,json=supportsDelta,proto3" json:"supports_delta,omitempty"`
	// Indicate the level of ramp support the device has
	RampSupport TweenSupport `protobuf:"varint,5,opt,name=ramp_support,json=rampSupport,proto3,enum=smartcore.bos.types.v1.TweenSupport" json:"ramp_support,omitempty"`
	// Provide information on how invalid values will be handled
	DefaultCapping *NumberCapping `protobuf:"bytes,6,opt,name=default_capping,json=defaultCapping,proto3" json:"default_capping,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *Int32Attributes) Reset() {
	*x = Int32Attributes{}
	mi := &file_smartcore_bos_types_v1_number_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *Int32Attributes) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*Int32Attributes) ProtoMessage() {}

func (x *Int32Attributes) ProtoReflect() protoreflect.Message {
	mi := &file_smartcore_bos_types_v1_number_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use Int32Attributes.ProtoReflect.Descriptor instead.
func (*Int32Attributes) Descriptor() ([]byte, []int) {
	return file_smartcore_bos_types_v1_number_proto_rawDescGZIP(), []int{2}
}

func (x *Int32Attributes) GetBounds() *Int32Bounds {
	if x != nil {
		return x.Bounds
	}
	return nil
}

func (x *Int32Attributes) GetStep() int32 {
	if x != nil {
		return x.Step
	}
	return 0
}

func (x *Int32Attributes) GetSupportsDelta() bool {
	if x != nil {
		return x.SupportsDelta
	}
	return false
}

func (x *Int32Attributes) GetRampSupport() TweenSupport {
	if x != nil {
		return x.RampSupport
	}
	return TweenSupport_TWEEN_SUPPORT_UNSPECIFIED
}

func (x *Int32Attributes) GetDefaultCapping() *NumberCapping {
	if x != nil {
		return x.DefaultCapping
	}
	return nil
}

// Defines the bounds for a number. Either of the min or max values can be absent which means those aspects are
// unbounded. It makes no sense for both to be unset, if so then this is no longer a bounds but that isn't
// disallowed.
//
// Contrary to most programming practices both min and max are inclusive and denote the max allowed value, just like
// htmls input type=range.
type FloatBounds struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// the minimum value (inclusive). If absent then there is no minimum, though this is typically Float.Min
	Min *float32 `protobuf:"fixed32,1,opt,name=min,proto3,oneof" json:"min,omitempty"`
	// the maximum value (inclusive). If absent then there is no maximum, though this is typically Float.Max
	Max           *float32 `protobuf:"fixed32,2,opt,name=max,proto3,oneof" json:"max,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *FloatBounds) Reset() {
	*x = FloatBounds{}
	mi := &file_smartcore_bos_types_v1_number_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FloatBounds) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FloatBounds) ProtoMessage() {}

func (x *FloatBounds) ProtoReflect() protoreflect.Message {
	mi := &file_smartcore_bos_types_v1_number_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FloatBounds.ProtoReflect.Descriptor instead.
func (*FloatBounds) Descriptor() ([]byte, []int) {
	return file_smartcore_bos_types_v1_number_proto_rawDescGZIP(), []int{3}
}

func (x *FloatBounds) GetMin() float32 {
	if x != nil && x.Min != nil {
		return *x.Min
	}
	return 0
}

func (x *FloatBounds) GetMax() float32 {
	if x != nil && x.Max != nil {
		return *x.Max
	}
	return 0
}

// Describes features and restrictions applied to a number that is typically used as part of an apis value
type FloatAttributes struct {
	state  protoimpl.MessageState `protogen:"open.v1"`
	Bounds *FloatBounds           `protobuf:"bytes,1,opt,name=bounds,proto3" json:"bounds,omitempty"`
	// The stepping interval, how little the value can change in one go. A default value of 0 is equivalent to a
	// continuous step, i.e. all possible float values are supported
	Step float32 `protobuf:"fixed32,3,opt,name=step,proto3" json:"step,omitempty"`
	// Indicate whether the device supports delta adjustments when changing the value
	SupportsDelta bool `protobuf:"varint,4,opt,name=supports_delta,json=supportsDelta,proto3" json:"supports_delta,omitempty"`
	// Indicate the level of ramp support the device has
	RampSupport TweenSupport `protobuf:"varint,5,opt,name=ramp_support,json=rampSupport,proto3,enum=smartcore.bos.types.v1.TweenSupport" json:"ramp_support,omitempty"`
	// Provide information on how invalid values will be handled
	DefaultCapping *NumberCapping `protobuf:"bytes,6,opt,name=default_capping,json=defaultCapping,proto3" json:"default_capping,omitempty"`
	unknownFields  protoimpl.UnknownFields
	sizeCache      protoimpl.SizeCache
}

func (x *FloatAttributes) Reset() {
	*x = FloatAttributes{}
	mi := &file_smartcore_bos_types_v1_number_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *FloatAttributes) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*FloatAttributes) ProtoMessage() {}

func (x *FloatAttributes) ProtoReflect() protoreflect.Message {
	mi := &file_smartcore_bos_types_v1_number_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use FloatAttributes.ProtoReflect.Descriptor instead.
func (*FloatAttributes) Descriptor() ([]byte, []int) {
	return file_smartcore_bos_types_v1_number_proto_rawDescGZIP(), []int{4}
}

func (x *FloatAttributes) GetBounds() *FloatBounds {
	if x != nil {
		return x.Bounds
	}
	return nil
}

func (x *FloatAttributes) GetStep() float32 {
	if x != nil {
		return x.Step
	}
	return 0
}

func (x *FloatAttributes) GetSupportsDelta() bool {
	if x != nil {
		return x.SupportsDelta
	}
	return false
}

func (x *FloatAttributes) GetRampSupport() TweenSupport {
	if x != nil {
		return x.RampSupport
	}
	return TweenSupport_TWEEN_SUPPORT_UNSPECIFIED
}

func (x *FloatAttributes) GetDefaultCapping() *NumberCapping {
	if x != nil {
		return x.DefaultCapping
	}
	return nil
}

var File_smartcore_bos_types_v1_number_proto protoreflect.FileDescriptor

const file_smartcore_bos_types_v1_number_proto_rawDesc = "" +
	"\n" +
	"#smartcore/bos/types/v1/number.proto\x12\x16smartcore.bos.types.v1\x1a\"smartcore/bos/types/v1/tween.proto\"\xd7\x01\n" +
	"\rNumberCapping\x12@\n" +
	"\x03min\x18\x01 \x01(\x0e2..smartcore.bos.types.v1.InvalidNumberBehaviourR\x03min\x12B\n" +
	"\x04step\x18\x02 \x01(\x0e2..smartcore.bos.types.v1.InvalidNumberBehaviourR\x04step\x12@\n" +
	"\x03max\x18\x03 \x01(\x0e2..smartcore.bos.types.v1.InvalidNumberBehaviourR\x03max\"K\n" +
	"\vInt32Bounds\x12\x15\n" +
	"\x03min\x18\x01 \x01(\x05H\x00R\x03min\x88\x01\x01\x12\x15\n" +
	"\x03max\x18\x02 \x01(\x05H\x01R\x03max\x88\x01\x01B\x06\n" +
	"\x04_minB\x06\n" +
	"\x04_max\"\xa2\x02\n" +
	"\x0fInt32Attributes\x12;\n" +
	"\x06bounds\x18\x01 \x01(\v2#.smartcore.bos.types.v1.Int32BoundsR\x06bounds\x12\x12\n" +
	"\x04step\x18\x03 \x01(\x05R\x04step\x12%\n" +
	"\x0esupports_delta\x18\x04 \x01(\bR\rsupportsDelta\x12G\n" +
	"\framp_support\x18\x05 \x01(\x0e2$.smartcore.bos.types.v1.TweenSupportR\vrampSupport\x12N\n" +
	"\x0fdefault_capping\x18\x06 \x01(\v2%.smartcore.bos.types.v1.NumberCappingR\x0edefaultCapping\"K\n" +
	"\vFloatBounds\x12\x15\n" +
	"\x03min\x18\x01 \x01(\x02H\x00R\x03min\x88\x01\x01\x12\x15\n" +
	"\x03max\x18\x02 \x01(\x02H\x01R\x03max\x88\x01\x01B\x06\n" +
	"\x04_minB\x06\n" +
	"\x04_max\"\xa2\x02\n" +
	"\x0fFloatAttributes\x12;\n" +
	"\x06bounds\x18\x01 \x01(\v2#.smartcore.bos.types.v1.FloatBoundsR\x06bounds\x12\x12\n" +
	"\x04step\x18\x03 \x01(\x02R\x04step\x12%\n" +
	"\x0esupports_delta\x18\x04 \x01(\bR\rsupportsDelta\x12G\n" +
	"\framp_support\x18\x05 \x01(\x0e2$.smartcore.bos.types.v1.TweenSupportR\vrampSupport\x12N\n" +
	"\x0fdefault_capping\x18\x06 \x01(\v2%.smartcore.bos.types.v1.NumberCappingR\x0edefaultCapping*f\n" +
	"\x16InvalidNumberBehaviour\x12(\n" +
	"$INVALID_NUMBER_BEHAVIOUR_UNSPECIFIED\x10\x00\x12\f\n" +
	"\bRESTRICT\x10\x01\x12\t\n" +
	"\x05ERROR\x10\x02\x12\t\n" +
	"\x05ALLOW\x10\x03Bi\n" +
	"\x13dev.smartcore.typesB\vNumberProtoP\x01Z1github.com/smart-core-os/sc-bos/pkg/proto/typespb\xaa\x02\x0fSmartcore.Typesb\x06proto3"

var (
	file_smartcore_bos_types_v1_number_proto_rawDescOnce sync.Once
	file_smartcore_bos_types_v1_number_proto_rawDescData []byte
)

func file_smartcore_bos_types_v1_number_proto_rawDescGZIP() []byte {
	file_smartcore_bos_types_v1_number_proto_rawDescOnce.Do(func() {
		file_smartcore_bos_types_v1_number_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_smartcore_bos_types_v1_number_proto_rawDesc), len(file_smartcore_bos_types_v1_number_proto_rawDesc)))
	})
	return file_smartcore_bos_types_v1_number_proto_rawDescData
}

var file_smartcore_bos_types_v1_number_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_smartcore_bos_types_v1_number_proto_msgTypes = make([]protoimpl.MessageInfo, 5)
var file_smartcore_bos_types_v1_number_proto_goTypes = []any{
	(InvalidNumberBehaviour)(0), // 0: smartcore.bos.types.v1.InvalidNumberBehaviour
	(*NumberCapping)(nil),       // 1: smartcore.bos.types.v1.NumberCapping
	(*Int32Bounds)(nil),         // 2: smartcore.bos.types.v1.Int32Bounds
	(*Int32Attributes)(nil),     // 3: smartcore.bos.types.v1.Int32Attributes
	(*FloatBounds)(nil),         // 4: smartcore.bos.types.v1.FloatBounds
	(*FloatAttributes)(nil),     // 5: smartcore.bos.types.v1.FloatAttributes
	(TweenSupport)(0),           // 6: smartcore.bos.types.v1.TweenSupport
}
var file_smartcore_bos_types_v1_number_proto_depIdxs = []int32{
	0, // 0: smartcore.bos.types.v1.NumberCapping.min:type_name -> smartcore.bos.types.v1.InvalidNumberBehaviour
	0, // 1: smartcore.bos.types.v1.NumberCapping.step:type_name -> smartcore.bos.types.v1.InvalidNumberBehaviour
	0, // 2: smartcore.bos.types.v1.NumberCapping.max:type_name -> smartcore.bos.types.v1.InvalidNumberBehaviour
	2, // 3: smartcore.bos.types.v1.Int32Attributes.bounds:type_name -> smartcore.bos.types.v1.Int32Bounds
	6, // 4: smartcore.bos.types.v1.Int32Attributes.ramp_support:type_name -> smartcore.bos.types.v1.TweenSupport
	1, // 5: smartcore.bos.types.v1.Int32Attributes.default_capping:type_name -> smartcore.bos.types.v1.NumberCapping
	4, // 6: smartcore.bos.types.v1.FloatAttributes.bounds:type_name -> smartcore.bos.types.v1.FloatBounds
	6, // 7: smartcore.bos.types.v1.FloatAttributes.ramp_support:type_name -> smartcore.bos.types.v1.TweenSupport
	1, // 8: smartcore.bos.types.v1.FloatAttributes.default_capping:type_name -> smartcore.bos.types.v1.NumberCapping
	9, // [9:9] is the sub-list for method output_type
	9, // [9:9] is the sub-list for method input_type
	9, // [9:9] is the sub-list for extension type_name
	9, // [9:9] is the sub-list for extension extendee
	0, // [0:9] is the sub-list for field type_name
}

func init() { file_smartcore_bos_types_v1_number_proto_init() }
func file_smartcore_bos_types_v1_number_proto_init() {
	if File_smartcore_bos_types_v1_number_proto != nil {
		return
	}
	file_smartcore_bos_types_v1_tween_proto_init()
	file_smartcore_bos_types_v1_number_proto_msgTypes[1].OneofWrappers = []any{}
	file_smartcore_bos_types_v1_number_proto_msgTypes[3].OneofWrappers = []any{}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_smartcore_bos_types_v1_number_proto_rawDesc), len(file_smartcore_bos_types_v1_number_proto_rawDesc)),
			NumEnums:      1,
			NumMessages:   5,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_smartcore_bos_types_v1_number_proto_goTypes,
		DependencyIndexes: file_smartcore_bos_types_v1_number_proto_depIdxs,
		EnumInfos:         file_smartcore_bos_types_v1_number_proto_enumTypes,
		MessageInfos:      file_smartcore_bos_types_v1_number_proto_msgTypes,
	}.Build()
	File_smartcore_bos_types_v1_number_proto = out.File
	file_smartcore_bos_types_v1_number_proto_goTypes = nil
	file_smartcore_bos_types_v1_number_proto_depIdxs = nil
}
