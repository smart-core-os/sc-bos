...
// ConvertUnit converts from one consumable unit to another, if possible.
// Units can only be converted if within the same category, LITER -> CUBIC_METER, but not LITER -> METER.
// Returns an error if conversion is not possible.
func ConvertUnit(v float64, from, to Consumable_Unit) (float64, error) {
	if from == to {
		return v, nil
	}
...
// ConvertUnit32 is like ConvertUnit with type conversion to float32.
func ConvertUnit32(v float32, from, to Consumable_Unit) (float32, error) {
	f64, err := ConvertUnit(float64(v), from, to)
...
var siUnits = map[Consumable_Unit]conv{
	Consumable_METER:       {meter, length},
	Consumable_LITER:       {liter, volume},
	Consumable_CUBIC_METER: {cubicMeter, volume},
	Consumable_CUP:         {usFluidCup, volume},
	Consumable_KILOGRAM:    {kilogram, weight},
}
