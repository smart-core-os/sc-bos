...
func TestSameValueType(t *testing.T) {
	tests := []struct {
		name string
		vals []*HealthCheck_Value
		want bool
	}{
		{"empty", nil, true},
		{"one", []*HealthCheck_Value{IntValue(1)}, true},
		{"many", []*HealthCheck_Value{IntValue(1), IntValue(2), IntValue(3)}, true},
		{"int", []*HealthCheck_Value{IntValue(1), IntValue(2), IntValue(3)}, true},
		{"uint", []*HealthCheck_Value{UintValue(1), UintValue(2), UintValue(3)}, true},
		{"float", []*HealthCheck_Value{FloatValue(1.5), FloatValue(2.5), FloatValue(3.5)}, true},
		{"bool", []*HealthCheck_Value{BoolValue(true), BoolValue(false), BoolValue(true)}, true},
		{"string", []*HealthCheck_Value{StringValue("a"), StringValue("b"), StringValue("c")}, true},
		{"timestamp", []*HealthCheck_Value{TimestampValue(time.Now()), TimestampValue(time.Now()), TimestampValue(time.Now())}, true},
		{"duration", []*HealthCheck_Value{DurationValue(1 * time.Second), DurationValue(2 * time.Second), DurationValue(3 * time.Second)}, true},
		// mixed types
		{"i,f,i", []*HealthCheck_Value{IntValue(1), FloatValue(2.5), IntValue(3)}, false},
		{"t,d", []*HealthCheck_Value{TimestampValue(time.Now()), DurationValue(2 * time.Second)}, false},
		{"s,s,i", []*HealthCheck_Value{StringValue("a"), StringValue("b"), IntValue(3)}, false},
	}
...
func TestAddValues(t *testing.T) {
	...
	tests := []struct {
		name string
		v, d *HealthCheck_Value
		want *HealthCheck_Value
	}{
