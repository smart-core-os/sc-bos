...
func ValidateValueRange(r *HealthCheck_ValueRange) error {
	if r == nil {
	...
	if bb := r.GetDeadband(); bb != nil {
		...
		case *HealthCheck_Value_TimestampValue:
			...
			if _, durVal := bb.GetValue().(*HealthCheck_Value_DurationValue); !durVal {
	...
	if _, ok := b.GetValue().(*HealthCheck_Value_BoolValue); ok {
...
func checkRangeState(r *HealthCheck_ValueRange, v *HealthCheck_Value, current HealthCheck_Normality) HealthCheck_Normality {
	switch current {
	case HealthCheck_LOW:
		if low := r.GetLow(); low != nil && less(v, AddValues(low, r.GetDeadband())) {
			return HealthCheck_LOW
		}
		if high := r.GetHigh(); high != nil && less(high, v) {
			return HealthCheck_HIGH
		}
		return HealthCheck_NORMAL
...
func less(x, y *HealthCheck_Value) bool {
	switch v := x.GetValue().(type) {
	case *HealthCheck_Value_StringValue:
...
func newBoundsCheck(c *HealthCheck) (*BoundsCheck, error) {
...
func (c *BoundsCheck) UpdateBounds(_ context.Context, b *HealthCheck_Bounds) error {
...
func (c *BoundsCheck) UpdateValue(_ context.Context, v *HealthCheck_Value) {
	c.write(func(dst *HealthCheck) {
		c.writeValue(dst, v)
		makeReliable(dst)
	})
...
func (c *BoundsCheck) writeValue(dst *HealthCheck, v *HealthCheck_Value) {
	if c.checker == nil {
		return
	}
	out := dst.GetBounds()
...
func newNormalValueCheck(v, nv *HealthCheck_Value) (*valueCheck, error) {
	...
	return &valueCheck{value: nv, eq: HealthCheck_NORMAL, neq: HealthCheck_ABNORMAL}, nil
}

func newAbnormalValueCheck(v, av *HealthCheck_Value) (*valueCheck, error) {
	...
	return &valueCheck{value: av, eq: HealthCheck_ABNORMAL, neq: HealthCheck_NORMAL}, nil
}

// valueCheck checks if a value matches a specific normal value.
type valueCheck struct {
	value   *HealthCheck_Value
	eq, neq HealthCheck_Normality
}

func (c *valueCheck) valueToState(v *HealthCheck_Value, _ *HealthCheck) HealthCheck_Normality {
...
func newNormalValuesCheck(v *HealthCheck_Value, vs []*HealthCheck_Value) (*valuesCheck, error) {
	...
	return &valuesCheck{values: vs, in: HealthCheck_NORMAL, nin: HealthCheck_ABNORMAL}, nil
}

func newAbnormalValuesCheck(v *HealthCheck_Value, vs []*HealthCheck_Value) (*valuesCheck, error) {
	if err := validateValuesCheck(v, vs); err != nil {
		return nil, fmt.Errorf("abnormal %w", err)
	}
	return &valuesCheck{values: vs, in: HealthCheck_ABNORMAL, nin: HealthCheck_NORMAL}, nil
}

// valuesCheck checks if a value matches any of a set of normal values.
type valuesCheck struct {
	values  []*HealthCheck_Value
	in, nin HealthCheck_Normality
}
...
func validateValuesCheck(v *HealthCheck_Value, vs []*HealthCheck_Value) error {
	if len(vs) == 0 {
...
func valuesToState(v *HealthCheck_Value, vs []*HealthCheck_Value, eq, neq HealthCheck_Normality) HealthCheck_Normality {
	for _, nv := range vs {
		if proto.Equal(v, nv) {
			return eq
		}
