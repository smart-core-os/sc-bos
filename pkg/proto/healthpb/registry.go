...
type Registry struct {
	...
	onCheckCreate func(name string, c *HealthCheck) *HealthCheck
	onCheckUpdate func(name string, c *HealthCheck)
...
func WithOnCheckCreate(f func(name string, c *HealthCheck) *HealthCheck) RegistryOption {
	return registryOptionFunc(func(r *Registry) {
		r.onCheckCreate = f
	})
}
...
func WithOnCheckUpdate(f func(name string, c *HealthCheck)) RegistryOption {
	return registryOptionFunc(func(r *Registry) {
		r.onCheckUpdate = f
	})
}
...
func (r *Registry) GetCheck(name, id string) *HealthCheck {
	r.mu.RLock()
	defer r.mu.RUnlock()
	nc, ok := r.byName[name]
	if !ok {
...
func (r *Registry) addCheck(name, id string, c *checkBase) error {
	...
	c.onCommit = func(c *HealthCheck) {
		if r.onCheckUpdate != nil {
			r.onCheckUpdate(name, c)
		}
	}
	c.onDispose = func(c *HealthCheck) {
...
func (r *Registry) initHealthCheck(hc *HealthCheck) {
...
func (hc *Checks) NewBoundsCheck(name string, c *HealthCheck) (*BoundsCheck, error) {
	hc.adjustId(c)
...
func (hc *Checks) NewFaultCheck(name string, c *HealthCheck) (*FaultCheck, error) {
	hc.adjustId(c)
...
func (hc *Checks) adjustId(c *HealthCheck) {
