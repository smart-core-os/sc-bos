package speakerpb

import (
	"context"

	"google.golang.org/protobuf/types/known/timestamppb"

	"github.com/smart-core-os/sc-bos/pkg/proto/typespb"
	"github.com/smart-core-os/sc-bos/pkg/resource"

	"google.golang.org/grpc"
	"google.golang.org/protobuf/proto"
)

type MemoryDevice struct {
	UnimplementedSpeakerApiServer
	volume *resource.Value
}

func NewMemoryDevice(initialState *typespb.AudioLevel) *MemoryDevice {
	return &MemoryDevice{
		volume: resource.NewValue(resource.WithInitialValue(initialState)),
	}
}

func (s *MemoryDevice) Register(server grpc.ServiceRegistrar) {
	RegisterSpeakerApiServer(server, s)
}

func (s *MemoryDevice) GetVolume(_ context.Context, req *GetSpeakerVolumeRequest) (*typespb.AudioLevel, error) {
	return s.volume.Get(resource.WithReadMask(req.ReadMask)).(*typespb.AudioLevel), nil
}

func (s *MemoryDevice) UpdateVolume(_ context.Context, request *UpdateSpeakerVolumeRequest) (*typespb.AudioLevel, error) {
	newValue, err := s.volume.Set(request.Volume, resource.WithUpdateMask(request.UpdateMask), resource.InterceptBefore(func(old, change proto.Message) {
		if request.Delta {
			val := old.(*typespb.AudioLevel)
			delta := change.(*typespb.AudioLevel)
			delta.Gain += val.Gain
		}
	}))
	if err != nil {
		return nil, err
	}
	return newValue.(*typespb.AudioLevel), nil
}

func (s *MemoryDevice) PullVolume(request *PullSpeakerVolumeRequest, server SpeakerApi_PullVolumeServer) error {
	for change := range s.volume.Pull(server.Context(), resource.WithReadMask(request.ReadMask), resource.WithUpdatesOnly(request.UpdatesOnly)) {
		typedChange := &typespb.AudioLevelChange{
			Name:       request.Name,
			Level:      change.Value.(*typespb.AudioLevel),
			ChangeTime: timestamppb.New(change.ChangeTime),
		}
		err := server.Send(&PullSpeakerVolumeResponse{
			Changes: []*typespb.AudioLevelChange{typedChange},
		})
		if err != nil {
			return err
		}
	}

	return server.Context().Err()
}
