// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        v7.34.0
// source: smartcore/bos/onoff/v1/on_off.proto

package onoffpb

import (
	typespb "github.com/smart-core-os/sc-bos/pkg/proto/typespb"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	fieldmaskpb "google.golang.org/protobuf/types/known/fieldmaskpb"
	timestamppb "google.golang.org/protobuf/types/known/timestamppb"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// Possible states for an On-Off device
type OnOff_State int32

const (
	OnOff_STATE_UNSPECIFIED OnOff_State = 0
	OnOff_ON                OnOff_State = 1
	OnOff_OFF               OnOff_State = 2
)

// Enum value maps for OnOff_State.
var (
	OnOff_State_name = map[int32]string{
		0: "STATE_UNSPECIFIED",
		1: "ON",
		2: "OFF",
	}
	OnOff_State_value = map[string]int32{
		"STATE_UNSPECIFIED": 0,
		"ON":                1,
		"OFF":               2,
	}
)

func (x OnOff_State) Enum() *OnOff_State {
	p := new(OnOff_State)
	*p = x
	return p
}

func (x OnOff_State) String() string {
	return protoimpl.X.EnumStringOf(x.Descriptor(), protoreflect.EnumNumber(x))
}

func (OnOff_State) Descriptor() protoreflect.EnumDescriptor {
	return file_smartcore_bos_onoff_v1_on_off_proto_enumTypes[0].Descriptor()
}

func (OnOff_State) Type() protoreflect.EnumType {
	return &file_smartcore_bos_onoff_v1_on_off_proto_enumTypes[0]
}

func (x OnOff_State) Number() protoreflect.EnumNumber {
	return protoreflect.EnumNumber(x)
}

// Deprecated: Use OnOff_State.Descriptor instead.
func (OnOff_State) EnumDescriptor() ([]byte, []int) {
	return file_smartcore_bos_onoff_v1_on_off_proto_rawDescGZIP(), []int{0, 0}
}

type OnOff struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	State         OnOff_State            `protobuf:"varint,1,opt,name=state,proto3,enum=smartcore.bos.onoff.v1.OnOff_State" json:"state,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OnOff) Reset() {
	*x = OnOff{}
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OnOff) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OnOff) ProtoMessage() {}

func (x *OnOff) ProtoReflect() protoreflect.Message {
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OnOff.ProtoReflect.Descriptor instead.
func (*OnOff) Descriptor() ([]byte, []int) {
	return file_smartcore_bos_onoff_v1_on_off_proto_rawDescGZIP(), []int{0}
}

func (x *OnOff) GetState() OnOff_State {
	if x != nil {
		return x.State
	}
	return OnOff_STATE_UNSPECIFIED
}

// OnOffSupport describes the capabilities of devices implementing this trait
type OnOffSupport struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// How a named device supports read/write/pull apis
	ResourceSupport *typespb.ResourceSupport `protobuf:"bytes,1,opt,name=resource_support,json=resourceSupport,proto3" json:"resource_support,omitempty"`
	unknownFields   protoimpl.UnknownFields
	sizeCache       protoimpl.SizeCache
}

func (x *OnOffSupport) Reset() {
	*x = OnOffSupport{}
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OnOffSupport) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OnOffSupport) ProtoMessage() {}

func (x *OnOffSupport) ProtoReflect() protoreflect.Message {
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OnOffSupport.ProtoReflect.Descriptor instead.
func (*OnOffSupport) Descriptor() ([]byte, []int) {
	return file_smartcore_bos_onoff_v1_on_off_proto_rawDescGZIP(), []int{1}
}

func (x *OnOffSupport) GetResourceSupport() *typespb.ResourceSupport {
	if x != nil {
		return x.ResourceSupport
	}
	return nil
}

type GetOnOffRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Name  string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// The state fields to fetch
	ReadMask      *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=read_mask,json=readMask,proto3" json:"read_mask,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *GetOnOffRequest) Reset() {
	*x = GetOnOffRequest{}
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *GetOnOffRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*GetOnOffRequest) ProtoMessage() {}

func (x *GetOnOffRequest) ProtoReflect() protoreflect.Message {
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use GetOnOffRequest.ProtoReflect.Descriptor instead.
func (*GetOnOffRequest) Descriptor() ([]byte, []int) {
	return file_smartcore_bos_onoff_v1_on_off_proto_rawDescGZIP(), []int{2}
}

func (x *GetOnOffRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *GetOnOffRequest) GetReadMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.ReadMask
	}
	return nil
}

type UpdateOnOffRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Name  string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	OnOff *OnOff                 `protobuf:"bytes,2,opt,name=on_off,json=onOff,proto3" json:"on_off,omitempty"`
	// The state fields to update
	UpdateMask    *fieldmaskpb.FieldMask `protobuf:"bytes,3,opt,name=update_mask,json=updateMask,proto3" json:"update_mask,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *UpdateOnOffRequest) Reset() {
	*x = UpdateOnOffRequest{}
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *UpdateOnOffRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*UpdateOnOffRequest) ProtoMessage() {}

func (x *UpdateOnOffRequest) ProtoReflect() protoreflect.Message {
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use UpdateOnOffRequest.ProtoReflect.Descriptor instead.
func (*UpdateOnOffRequest) Descriptor() ([]byte, []int) {
	return file_smartcore_bos_onoff_v1_on_off_proto_rawDescGZIP(), []int{3}
}

func (x *UpdateOnOffRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *UpdateOnOffRequest) GetOnOff() *OnOff {
	if x != nil {
		return x.OnOff
	}
	return nil
}

func (x *UpdateOnOffRequest) GetUpdateMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.UpdateMask
	}
	return nil
}

// A request to monitor the state of a device
type PullOnOffRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Name  string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	// The state fields to fetch
	ReadMask *fieldmaskpb.FieldMask `protobuf:"bytes,2,opt,name=read_mask,json=readMask,proto3" json:"read_mask,omitempty"`
	// When true the device will only send changes to the resource value.
	// The default behaviour is to send the current value immediately followed by any updates as they happen.
	UpdatesOnly   bool `protobuf:"varint,3,opt,name=updates_only,json=updatesOnly,proto3" json:"updates_only,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PullOnOffRequest) Reset() {
	*x = PullOnOffRequest{}
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[4]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PullOnOffRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PullOnOffRequest) ProtoMessage() {}

func (x *PullOnOffRequest) ProtoReflect() protoreflect.Message {
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[4]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PullOnOffRequest.ProtoReflect.Descriptor instead.
func (*PullOnOffRequest) Descriptor() ([]byte, []int) {
	return file_smartcore_bos_onoff_v1_on_off_proto_rawDescGZIP(), []int{4}
}

func (x *PullOnOffRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *PullOnOffRequest) GetReadMask() *fieldmaskpb.FieldMask {
	if x != nil {
		return x.ReadMask
	}
	return nil
}

func (x *PullOnOffRequest) GetUpdatesOnly() bool {
	if x != nil {
		return x.UpdatesOnly
	}
	return false
}

// Response to a Pull request for OnOffState changes
type PullOnOffResponse struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Changes that have occurred since the last event
	Changes       []*PullOnOffResponse_Change `protobuf:"bytes,1,rep,name=changes,proto3" json:"changes,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PullOnOffResponse) Reset() {
	*x = PullOnOffResponse{}
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[5]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PullOnOffResponse) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PullOnOffResponse) ProtoMessage() {}

func (x *PullOnOffResponse) ProtoReflect() protoreflect.Message {
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[5]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PullOnOffResponse.ProtoReflect.Descriptor instead.
func (*PullOnOffResponse) Descriptor() ([]byte, []int) {
	return file_smartcore_bos_onoff_v1_on_off_proto_rawDescGZIP(), []int{5}
}

func (x *PullOnOffResponse) GetChanges() []*PullOnOffResponse_Change {
	if x != nil {
		return x.Changes
	}
	return nil
}

type DescribeOnOffRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The name of the device
	Name          string `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *DescribeOnOffRequest) Reset() {
	*x = DescribeOnOffRequest{}
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[6]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *DescribeOnOffRequest) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*DescribeOnOffRequest) ProtoMessage() {}

func (x *DescribeOnOffRequest) ProtoReflect() protoreflect.Message {
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[6]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use DescribeOnOffRequest.ProtoReflect.Descriptor instead.
func (*DescribeOnOffRequest) Descriptor() ([]byte, []int) {
	return file_smartcore_bos_onoff_v1_on_off_proto_rawDescGZIP(), []int{6}
}

func (x *DescribeOnOffRequest) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

type PullOnOffResponse_Change struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	Name  string                 `protobuf:"bytes,1,opt,name=name,proto3" json:"name,omitempty"`
	OnOff *OnOff                 `protobuf:"bytes,2,opt,name=on_off,json=onOff,proto3" json:"on_off,omitempty"`
	// when the change occurred
	ChangeTime    *timestamppb.Timestamp `protobuf:"bytes,3,opt,name=change_time,json=changeTime,proto3" json:"change_time,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *PullOnOffResponse_Change) Reset() {
	*x = PullOnOffResponse_Change{}
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[7]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *PullOnOffResponse_Change) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*PullOnOffResponse_Change) ProtoMessage() {}

func (x *PullOnOffResponse_Change) ProtoReflect() protoreflect.Message {
	mi := &file_smartcore_bos_onoff_v1_on_off_proto_msgTypes[7]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use PullOnOffResponse_Change.ProtoReflect.Descriptor instead.
func (*PullOnOffResponse_Change) Descriptor() ([]byte, []int) {
	return file_smartcore_bos_onoff_v1_on_off_proto_rawDescGZIP(), []int{5, 0}
}

func (x *PullOnOffResponse_Change) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *PullOnOffResponse_Change) GetOnOff() *OnOff {
	if x != nil {
		return x.OnOff
	}
	return nil
}

func (x *PullOnOffResponse_Change) GetChangeTime() *timestamppb.Timestamp {
	if x != nil {
		return x.ChangeTime
	}
	return nil
}

var File_smartcore_bos_onoff_v1_on_off_proto protoreflect.FileDescriptor

const file_smartcore_bos_onoff_v1_on_off_proto_rawDesc = "" +
	"\n" +
	"#smartcore/bos/onoff/v1/on_off.proto\x12\x16smartcore.bos.onoff.v1\x1a google/protobuf/field_mask.proto\x1a\x1fgoogle/protobuf/timestamp.proto\x1a!smartcore/bos/types/v1/info.proto\"s\n" +
	"\x05OnOff\x129\n" +
	"\x05state\x18\x01 \x01(\x0e2#.smartcore.bos.onoff.v1.OnOff.StateR\x05state\"/\n" +
	"\x05State\x12\x15\n" +
	"\x11STATE_UNSPECIFIED\x10\x00\x12\x06\n" +
	"\x02ON\x10\x01\x12\a\n" +
	"\x03OFF\x10\x02\"b\n" +
	"\fOnOffSupport\x12R\n" +
	"\x10resource_support\x18\x01 \x01(\v2'.smartcore.bos.types.v1.ResourceSupportR\x0fresourceSupport\"^\n" +
	"\x0fGetOnOffRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x127\n" +
	"\tread_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskR\breadMask\"\x9b\x01\n" +
	"\x12UpdateOnOffRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x124\n" +
	"\x06on_off\x18\x02 \x01(\v2\x1d.smartcore.bos.onoff.v1.OnOffR\x05onOff\x12;\n" +
	"\vupdate_mask\x18\x03 \x01(\v2\x1a.google.protobuf.FieldMaskR\n" +
	"updateMask\"\x82\x01\n" +
	"\x10PullOnOffRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x127\n" +
	"\tread_mask\x18\x02 \x01(\v2\x1a.google.protobuf.FieldMaskR\breadMask\x12!\n" +
	"\fupdates_only\x18\x03 \x01(\bR\vupdatesOnly\"\xf1\x01\n" +
	"\x11PullOnOffResponse\x12J\n" +
	"\achanges\x18\x01 \x03(\v20.smartcore.bos.onoff.v1.PullOnOffResponse.ChangeR\achanges\x1a\x8f\x01\n" +
	"\x06Change\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name\x124\n" +
	"\x06on_off\x18\x02 \x01(\v2\x1d.smartcore.bos.onoff.v1.OnOffR\x05onOff\x12;\n" +
	"\vchange_time\x18\x03 \x01(\v2\x1a.google.protobuf.TimestampR\n" +
	"changeTime\"*\n" +
	"\x14DescribeOnOffRequest\x12\x12\n" +
	"\x04name\x18\x01 \x01(\tR\x04name2\x9c\x02\n" +
	"\bOnOffApi\x12R\n" +
	"\bGetOnOff\x12'.smartcore.bos.onoff.v1.GetOnOffRequest\x1a\x1d.smartcore.bos.onoff.v1.OnOff\x12X\n" +
	"\vUpdateOnOff\x12*.smartcore.bos.onoff.v1.UpdateOnOffRequest\x1a\x1d.smartcore.bos.onoff.v1.OnOff\x12b\n" +
	"\tPullOnOff\x12(.smartcore.bos.onoff.v1.PullOnOffRequest\x1a).smartcore.bos.onoff.v1.PullOnOffResponse0\x012p\n" +
	"\tOnOffInfo\x12c\n" +
	"\rDescribeOnOff\x12,.smartcore.bos.onoff.v1.DescribeOnOffRequest\x1a$.smartcore.bos.onoff.v1.OnOffSupportB}\n" +
	"\x14dev.smartcore.traitsB\n" +
	"OnOffProtoP\x01Z1github.com/smart-core-os/sc-bos/pkg/proto/onoffpb\xaa\x02\x10Smartcore.Traits\xca\x02\x10Smartcore\\Traitsb\x06proto3"

var (
	file_smartcore_bos_onoff_v1_on_off_proto_rawDescOnce sync.Once
	file_smartcore_bos_onoff_v1_on_off_proto_rawDescData []byte
)

func file_smartcore_bos_onoff_v1_on_off_proto_rawDescGZIP() []byte {
	file_smartcore_bos_onoff_v1_on_off_proto_rawDescOnce.Do(func() {
		file_smartcore_bos_onoff_v1_on_off_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_smartcore_bos_onoff_v1_on_off_proto_rawDesc), len(file_smartcore_bos_onoff_v1_on_off_proto_rawDesc)))
	})
	return file_smartcore_bos_onoff_v1_on_off_proto_rawDescData
}

var file_smartcore_bos_onoff_v1_on_off_proto_enumTypes = make([]protoimpl.EnumInfo, 1)
var file_smartcore_bos_onoff_v1_on_off_proto_msgTypes = make([]protoimpl.MessageInfo, 8)
var file_smartcore_bos_onoff_v1_on_off_proto_goTypes = []any{
	(OnOff_State)(0),                 // 0: smartcore.bos.onoff.v1.OnOff.State
	(*OnOff)(nil),                    // 1: smartcore.bos.onoff.v1.OnOff
	(*OnOffSupport)(nil),             // 2: smartcore.bos.onoff.v1.OnOffSupport
	(*GetOnOffRequest)(nil),          // 3: smartcore.bos.onoff.v1.GetOnOffRequest
	(*UpdateOnOffRequest)(nil),       // 4: smartcore.bos.onoff.v1.UpdateOnOffRequest
	(*PullOnOffRequest)(nil),         // 5: smartcore.bos.onoff.v1.PullOnOffRequest
	(*PullOnOffResponse)(nil),        // 6: smartcore.bos.onoff.v1.PullOnOffResponse
	(*DescribeOnOffRequest)(nil),     // 7: smartcore.bos.onoff.v1.DescribeOnOffRequest
	(*PullOnOffResponse_Change)(nil), // 8: smartcore.bos.onoff.v1.PullOnOffResponse.Change
	(*typespb.ResourceSupport)(nil),  // 9: smartcore.bos.types.v1.ResourceSupport
	(*fieldmaskpb.FieldMask)(nil),    // 10: google.protobuf.FieldMask
	(*timestamppb.Timestamp)(nil),    // 11: google.protobuf.Timestamp
}
var file_smartcore_bos_onoff_v1_on_off_proto_depIdxs = []int32{
	0,  // 0: smartcore.bos.onoff.v1.OnOff.state:type_name -> smartcore.bos.onoff.v1.OnOff.State
	9,  // 1: smartcore.bos.onoff.v1.OnOffSupport.resource_support:type_name -> smartcore.bos.types.v1.ResourceSupport
	10, // 2: smartcore.bos.onoff.v1.GetOnOffRequest.read_mask:type_name -> google.protobuf.FieldMask
	1,  // 3: smartcore.bos.onoff.v1.UpdateOnOffRequest.on_off:type_name -> smartcore.bos.onoff.v1.OnOff
	10, // 4: smartcore.bos.onoff.v1.UpdateOnOffRequest.update_mask:type_name -> google.protobuf.FieldMask
	10, // 5: smartcore.bos.onoff.v1.PullOnOffRequest.read_mask:type_name -> google.protobuf.FieldMask
	8,  // 6: smartcore.bos.onoff.v1.PullOnOffResponse.changes:type_name -> smartcore.bos.onoff.v1.PullOnOffResponse.Change
	1,  // 7: smartcore.bos.onoff.v1.PullOnOffResponse.Change.on_off:type_name -> smartcore.bos.onoff.v1.OnOff
	11, // 8: smartcore.bos.onoff.v1.PullOnOffResponse.Change.change_time:type_name -> google.protobuf.Timestamp
	3,  // 9: smartcore.bos.onoff.v1.OnOffApi.GetOnOff:input_type -> smartcore.bos.onoff.v1.GetOnOffRequest
	4,  // 10: smartcore.bos.onoff.v1.OnOffApi.UpdateOnOff:input_type -> smartcore.bos.onoff.v1.UpdateOnOffRequest
	5,  // 11: smartcore.bos.onoff.v1.OnOffApi.PullOnOff:input_type -> smartcore.bos.onoff.v1.PullOnOffRequest
	7,  // 12: smartcore.bos.onoff.v1.OnOffInfo.DescribeOnOff:input_type -> smartcore.bos.onoff.v1.DescribeOnOffRequest
	1,  // 13: smartcore.bos.onoff.v1.OnOffApi.GetOnOff:output_type -> smartcore.bos.onoff.v1.OnOff
	1,  // 14: smartcore.bos.onoff.v1.OnOffApi.UpdateOnOff:output_type -> smartcore.bos.onoff.v1.OnOff
	6,  // 15: smartcore.bos.onoff.v1.OnOffApi.PullOnOff:output_type -> smartcore.bos.onoff.v1.PullOnOffResponse
	2,  // 16: smartcore.bos.onoff.v1.OnOffInfo.DescribeOnOff:output_type -> smartcore.bos.onoff.v1.OnOffSupport
	13, // [13:17] is the sub-list for method output_type
	9,  // [9:13] is the sub-list for method input_type
	9,  // [9:9] is the sub-list for extension type_name
	9,  // [9:9] is the sub-list for extension extendee
	0,  // [0:9] is the sub-list for field type_name
}

func init() { file_smartcore_bos_onoff_v1_on_off_proto_init() }
func file_smartcore_bos_onoff_v1_on_off_proto_init() {
	if File_smartcore_bos_onoff_v1_on_off_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_smartcore_bos_onoff_v1_on_off_proto_rawDesc), len(file_smartcore_bos_onoff_v1_on_off_proto_rawDesc)),
			NumEnums:      1,
			NumMessages:   8,
			NumExtensions: 0,
			NumServices:   2,
		},
		GoTypes:           file_smartcore_bos_onoff_v1_on_off_proto_goTypes,
		DependencyIndexes: file_smartcore_bos_onoff_v1_on_off_proto_depIdxs,
		EnumInfos:         file_smartcore_bos_onoff_v1_on_off_proto_enumTypes,
		MessageInfos:      file_smartcore_bos_onoff_v1_on_off_proto_msgTypes,
	}.Build()
	File_smartcore_bos_onoff_v1_on_off_proto = out.File
	file_smartcore_bos_onoff_v1_on_off_proto_goTypes = nil
	file_smartcore_bos_onoff_v1_on_off_proto_depIdxs = nil
}
