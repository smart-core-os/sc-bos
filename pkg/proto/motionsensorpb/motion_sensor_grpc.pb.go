...
const (
	MotionSensorApi_GetMotionDetection_FullMethodName   = "/smartcore.bos.motionsensor.v1.MotionSensorApi/GetMotionDetection"
	MotionSensorApi_PullMotionDetections_FullMethodName = "/smartcore.bos.motionsensor.v1.MotionSensorApi/PullMotionDetections"
)
...
var MotionSensorApi_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "smartcore.bos.motionsensor.v1.MotionSensorApi",
	...
	Metadata: "smartcore/bos/motionsensor/v1/motion_sensor.proto",
}

const (
	MotionSensorInfo_DescribeMotionDetection_FullMethodName = "/smartcore.bos.motionsensor.v1.MotionSensorInfo/DescribeMotionDetection"
)

// MotionSensorInfoClient is the client API for MotionSensorInfo service.
//
// For semantics around ctx use and closing/ending streaming RPCs, please refer to https://pkg.go.dev/google.golang.org/grpc/?tab=doc#ClientConn.NewStream.
//
// Describes the capabilities of a specific named device with respect to this trait.
type MotionSensorInfoClient interface {
...
type motionSensorInfoClient struct {
	cc grpc.ClientConnInterface
}

func NewMotionSensorInfoClient(cc grpc.ClientConnInterface) MotionSensorInfoClient {
	return &motionSensorInfoClient{cc}
}

func (c *motionSensorInfoClient) DescribeMotionDetection(ctx context.Context, in *DescribeMotionDetectionRequest, opts ...grpc.CallOption) (*MotionDetectionSupport, error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	out := new(MotionDetectionSupport)
	err := c.cc.Invoke(ctx, MotionSensorInfo_DescribeMotionDetection_FullMethodName, in, out, cOpts...)
	if err != nil {
		return nil, err
	}
	return out, nil
...
// MotionSensorInfoServer is the server API for MotionSensorInfo service.
// All implementations must embed UnimplementedMotionSensorInfoServer
// for forward compatibility.
//
// Describes the capabilities of a specific named device with respect to this trait.
type MotionSensorInfoServer interface {
	// Get information about how a named device implements MotionDetection measurements
	DescribeMotionDetection(context.Context, *DescribeMotionDetectionRequest) (*MotionDetectionSupport, error)
	mustEmbedUnimplementedMotionSensorInfoServer()
}

// UnimplementedMotionSensorInfoServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
type UnimplementedMotionSensorInfoServer struct{}

func (UnimplementedMotionSensorInfoServer) DescribeMotionDetection(context.Context, *DescribeMotionDetectionRequest) (*MotionDetectionSupport, error) {
	return nil, status.Errorf(codes.Unimplemented, "method DescribeMotionDetection not implemented")
}
func (UnimplementedMotionSensorInfoServer) mustEmbedUnimplementedMotionSensorInfoServer() {}
func (UnimplementedMotionSensorInfoServer) testEmbeddedByValue()                          {}

// UnsafeMotionSensorInfoServer may be embedded to opt out of forward compatibility for this service.
// Use of this interface is not recommended, as added methods to MotionSensorInfoServer will
// result in compilation errors.
type UnsafeMotionSensorInfoServer interface {
	mustEmbedUnimplementedMotionSensorInfoServer()
}

func RegisterMotionSensorInfoServer(s grpc.ServiceRegistrar, srv MotionSensorInfoServer) {
	// If the following call pancis, it indicates UnimplementedMotionSensorInfoServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
	...
	s.RegisterService(&MotionSensorInfo_ServiceDesc, srv)
}

func _MotionSensorInfo_DescribeMotionDetection_Handler(srv interface{}, ctx context.Context, dec func(interface{}) error, interceptor grpc.UnaryServerInterceptor) (interface{}, error) {
	in := new(DescribeMotionDetectionRequest)
	if err := dec(in); err != nil {
		return nil, err
	}
	if interceptor == nil {
		return srv.(MotionSensorInfoServer).DescribeMotionDetection(ctx, in)
	}
	info := &grpc.UnaryServerInfo{
		Server:     srv,
		FullMethod: MotionSensorInfo_DescribeMotionDetection_FullMethodName,
	}
	handler := func(ctx context.Context, req interface{}) (interface{}, error) {
		return srv.(MotionSensorInfoServer).DescribeMotionDetection(ctx, req.(*DescribeMotionDetectionRequest))
	}
	return interceptor(ctx, in, info, handler)
}

// MotionSensorInfo_ServiceDesc is the grpc.ServiceDesc for MotionSensorInfo service.
// It's only intended for direct use with grpc.RegisterService,
// and not to be introspected or modified (even as a copy)
var MotionSensorInfo_ServiceDesc = grpc.ServiceDesc{
	ServiceName: "smartcore.bos.motionsensor.v1.MotionSensorInfo",
	HandlerType: (*MotionSensorInfoServer)(nil),
	Methods: []grpc.MethodDesc{
		{
			MethodName: "DescribeMotionDetection",
			Handler:    _MotionSensorInfo_DescribeMotionDetection_Handler,
		},
	},
	Streams:  []grpc.StreamDesc{},
	Metadata: "smartcore/bos/motionsensor/v1/motion_sensor.proto",
