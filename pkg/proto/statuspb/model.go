...
func (m *Model) GetCurrentStatus(readMask *fieldmaskpb.FieldMask) (*StatusLog, error) {
	...
	return clone.(*StatusLog), nil
}

func (m *Model) problemsToStatusLog(problemMsgs []proto.Message) *StatusLog {
	pm := &ProblemMerger{}
...
func (m *Model) UpdateProblem(problem *StatusLog_Problem) (*StatusLog_Problem, error) {
	...
	res, err := m.problems.Update(problem.Name, problem, resource.WithCreateIfAbsent(), resource.InterceptAfter(func(old, new proto.Message) {
		...
		var oldp, newp *StatusLog_Problem
		oldp = old.(*StatusLog_Problem)
		newp = new.(*StatusLog_Problem)
	...
	return res.(*StatusLog_Problem), nil
}
...
func (m *Model) PullCurrentStatus(ctx context.Context, readMask *fieldmaskpb.FieldMask, updatesOnly bool) <-chan StatusLogChange {
	...
	go func() {
		...
		for change := range stream {
			...
			statusLog = filter.FilterClone(statusLog).(*StatusLog)
