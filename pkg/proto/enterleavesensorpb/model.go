...
func (m *Model) CreateEnterLeaveEvent(event *EnterLeaveEvent, opts ...resource.WriteOption) error {
	updateTotalsOption := resource.InterceptBefore(func(current, value proto.Message) {
		currentVal, valueVal := current.(*EnterLeaveEvent), value.(*EnterLeaveEvent)
		...
		valueVal.EnterTotal = adjustTotal(valueVal.EnterTotal, currentVal.EnterTotal, valueVal.Direction == EnterLeaveEvent_ENTER)
		valueVal.LeaveTotal = adjustTotal(valueVal.LeaveTotal, currentVal.LeaveTotal, valueVal.Direction == EnterLeaveEvent_LEAVE)
	})
...
func (m *Model) GetEnterLeaveEvent(opts ...resource.ReadOption) (*EnterLeaveEvent, error) {
	val := m.enterLeaveEvents.Get(opts...)
	return val.(*EnterLeaveEvent), nil
}

func (m *Model) ResetTotals() error {
	var zero int32
	_, err := m.enterLeaveEvents.Set(&EnterLeaveEvent{EnterTotal: &zero, LeaveTotal: &zero}, resource.WithUpdatePaths("enter_total", "leave_total"))
	return err
}
