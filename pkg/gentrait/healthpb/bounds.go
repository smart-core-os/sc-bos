...
func ValidateValueRange(r *healthpb.HealthCheck_ValueRange) error {
	if r == nil {
	...
	if bb := r.GetDeadband(); bb != nil {
		...
		case *healthpb.HealthCheck_Value_TimestampValue:
			...
			if _, durVal := bb.GetValue().(*healthpb.HealthCheck_Value_DurationValue); !durVal {
	...
	if _, ok := b.GetValue().(*healthpb.HealthCheck_Value_BoolValue); ok {
...
func checkRangeState(r *healthpb.HealthCheck_ValueRange, v *healthpb.HealthCheck_Value, current healthpb.HealthCheck_Normality) healthpb.HealthCheck_Normality {
	switch current {
	case healthpb.HealthCheck_LOW:
		if low := r.GetLow(); low != nil && less(v, AddValues(low, r.GetDeadband())) {
			return healthpb.HealthCheck_LOW
		}
		if high := r.GetHigh(); high != nil && less(high, v) {
			return healthpb.HealthCheck_HIGH
		}
		return healthpb.HealthCheck_NORMAL
	case healthpb.HealthCheck_HIGH:
...
func less(x, y *healthpb.HealthCheck_Value) bool {
	switch v := x.GetValue().(type) {
	case *healthpb.HealthCheck_Value_StringValue:
...
func newBoundsCheck(c *healthpb.HealthCheck) (*BoundsCheck, error) {
...
func (c *BoundsCheck) UpdateBounds(_ context.Context, b *healthpb.HealthCheck_Bounds) error {
...
func (c *BoundsCheck) UpdateValue(_ context.Context, v *healthpb.HealthCheck_Value) {
	c.write(func(dst *healthpb.HealthCheck) {
		c.writeValue(dst, v)
		makeReliable(dst)
	})
...
func (c *BoundsCheck) writeValue(dst *healthpb.HealthCheck, v *healthpb.HealthCheck_Value) {
	if c.checker == nil {
		return
	}
	out := dst.GetBounds()
...
func (c *BoundsCheck) prepareChecker(b *healthpb.HealthCheck_Bounds) (boundsChecker, error) {
	switch v := b.GetExpected().(type) {
	case nil:
		return nil, nil // no bounds, no value checking
	case *healthpb.HealthCheck_Bounds_NormalValue:
		return newNormalValueCheck(b.CurrentValue, v.NormalValue)
	case *healthpb.HealthCheck_Bounds_AbnormalValue:
		return newAbnormalValueCheck(b.CurrentValue, v.AbnormalValue)
	case *healthpb.HealthCheck_Bounds_NormalRange:
		return newNormalRangeCheck(b.CurrentValue, v.NormalRange)
	case *healthpb.HealthCheck_Bounds_NormalValues:
		return newNormalValuesCheck(b.CurrentValue, v.NormalValues.GetValues())
	case *healthpb.HealthCheck_Bounds_AbnormalValues:
...
type boundsChecker interface {
	valueToState(v *healthpb.HealthCheck_Value, current *healthpb.HealthCheck) healthpb.HealthCheck_Normality
}

func newNormalValueCheck(v, nv *healthpb.HealthCheck_Value) (*valueCheck, error) {
	...
	return &valueCheck{value: nv, eq: healthpb.HealthCheck_NORMAL, neq: healthpb.HealthCheck_ABNORMAL}, nil
}

func newAbnormalValueCheck(v, av *healthpb.HealthCheck_Value) (*valueCheck, error) {
	...
	return &valueCheck{value: av, eq: healthpb.HealthCheck_ABNORMAL, neq: healthpb.HealthCheck_NORMAL}, nil
}

// valueCheck checks if a value matches a specific normal value.
type valueCheck struct {
	value   *healthpb.HealthCheck_Value
	eq, neq healthpb.HealthCheck_Normality
}

func (c *valueCheck) valueToState(v *healthpb.HealthCheck_Value, _ *healthpb.HealthCheck) healthpb.HealthCheck_Normality {
...
func newNormalRangeCheck(v *healthpb.HealthCheck_Value, r *healthpb.HealthCheck_ValueRange) (*normalRangeCheck, error) {
...
func (c *normalRangeCheck) valueToState(v *healthpb.HealthCheck_Value, current *healthpb.HealthCheck) healthpb.HealthCheck_Normality {
	return checkRangeState(c.bounds, v, current.GetNormality())
}

func newNormalValuesCheck(v *healthpb.HealthCheck_Value, vs []*healthpb.HealthCheck_Value) (*valuesCheck, error) {
	if err := validateValuesCheck(v, vs); err != nil {
		return nil, fmt.Errorf("normal %w", err)
	}
	return &valuesCheck{values: vs, in: healthpb.HealthCheck_NORMAL, nin: healthpb.HealthCheck_ABNORMAL}, nil
}

func newAbnormalValuesCheck(v *healthpb.HealthCheck_Value, vs []*healthpb.HealthCheck_Value) (*valuesCheck, error) {
	if err := validateValuesCheck(v, vs); err != nil {
		return nil, fmt.Errorf("abnormal %w", err)
	}
	return &valuesCheck{values: vs, in: healthpb.HealthCheck_ABNORMAL, nin: healthpb.HealthCheck_NORMAL}, nil
}

// valuesCheck checks if a value matches any of a set of normal values.
type valuesCheck struct {
	values  []*healthpb.HealthCheck_Value
	in, nin healthpb.HealthCheck_Normality
}

func (c *valuesCheck) valueToState(v *healthpb.HealthCheck_Value, _ *healthpb.HealthCheck) healthpb.HealthCheck_Normality {
...
func validateValuesCheck(v *healthpb.HealthCheck_Value, vs []*healthpb.HealthCheck_Value) error {
	if len(vs) == 0 {
...
func valuesToState(v *healthpb.HealthCheck_Value, vs []*healthpb.HealthCheck_Value, eq, neq healthpb.HealthCheck_Normality) healthpb.HealthCheck_Normality {
	for _, nv := range vs {
		if proto.Equal(v, nv) {
			return eq
		}
