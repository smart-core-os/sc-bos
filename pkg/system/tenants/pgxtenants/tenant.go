...
func CreateTenant(ctx context.Context, tx pgx.Tx, title string) (*tenantpb.Tenant, error) {
	// language=postgresql
	query := `
...
func GetTenant(ctx context.Context, tx pgx.Tx, id string) (*tenantpb.Tenant, error) {
	// language=postgresql
	query := `
		SELECT title, create_time FROM tenant WHERE id = $1;
    `

	tenant := &tenantpb.Tenant{Id: id}
...
func ListTenants(ctx context.Context, tx pgx.Tx) ([]*tenantpb.Tenant, error) {
	// language=postgresql
	query := `
...
func CreateTenantSecret(ctx context.Context, tx pgx.Tx, secret *tenantpb.Secret) (*tenantpb.Secret, error) {
	secret = proto.Clone(secret).(*tenantpb.Secret)
...
// GetTenantSecret returns a tenantpb.Secret by secret id.
func GetTenantSecret(ctx context.Context, tx pgx.Tx, id string) (*tenantpb.Secret, error) {
	// language=postgresql
	query := `
...
func ListTenantSecrets(ctx context.Context, tx pgx.Tx, tenantID string) ([]*tenantpb.Secret, error) {
	// language=postgresql
	query := `
		SELECT s.id, s.tenant, t.title, s.secret_hash, s.note, s.create_time, s.expire_time, s.first_use_time, s.last_use_time
		FROM tenant_secret s
...
func scanTenantSecret(row pgx.Row, secret *tenantpb.Secret) error {
	if secret.Tenant == nil {
		secret.Tenant = &tenantpb.Tenant{}
	}
	var (
