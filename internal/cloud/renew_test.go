...
func TestConnRenew_AgainstSim(t *testing.T) {
	...
	conn, err := OpenConn(ctx, regStore, depStore, ts.URL,
	...
	before := conn.State().Registration.Leaf().SerialNumber
	...
	st := conn.State()
	if st.Registration.Leaf().SerialNumber.Cmp(before) == 0 {
		t.Error("expected a fresh certificate serial after renew")
	}
	if st.Registration.NodeID() != cred.NodeID() {
		t.Errorf("node id changed across renew: %q -> %q", cred.NodeID(), st.Registration.NodeID())
	}
	...
	reloaded, ok, err := regStore.Load(ctx)
	if err != nil || !ok {
		t.Fatalf("reload credential: ok=%v err=%v", ok, err)
	}
	if reloaded.Leaf().SerialNumber.Cmp(st.Registration.Leaf().SerialNumber) != 0 {
...
func TestConnRenew_RefreshesFailedState(t *testing.T) {
	...
	if _, err := conn.Register(ctx, testRegistration(t, "node-x")); err != nil {
		t.Fatalf("Register: %v", err)
	}
	...
	fake.setNext(testRegistration(t, "node-x"))
	if err := conn.Renew(ctx); err != nil {
		t.Fatalf("Renew: %v", err)
	}
...
func TestAutoRenew_FiresWhenDueAndReArms(t *testing.T) {
	...
	due := datedRegistration(t, "node-y", now.Add(-100*time.Hour), now.Add(20*time.Hour))
	if _, err := conn.Register(ctx, due); err != nil {
		t.Fatalf("Register: %v", err)
	}
	// The renewed certificate is freshly issued, so its renewal point is far off.
	fresh := datedRegistration(t, "node-y", now, now.Add(120*time.Hour))
	fake.setNext(fresh)

	go AutoRenew(ctx, conn, zap.NewNop())

	waitFor(t, 2*time.Second, func() bool {
		leaf := conn.State().Registration.Leaf()
...
func newStores(t *testing.T) (RegistrationStore, *DeploymentStore) {
	t.Helper()
	...
	return regStore, NewDeploymentStore(depRoot)
}

func newConnWithFake(t *testing.T, fake Client) *Conn {
	t.Helper()
	regStore, depStore := newStores(t)
	conn, err := OpenConn(context.Background(), regStore, depStore, "",
		WithClientFactory(func(*Registration) Client { return fake }))
	if err != nil {
		t.Fatalf("OpenConn: %v", err)
	}
	return conn
...
// datedRegistration builds a self-signed credential whose leaf CN is nodeID and
// whose validity window is exactly [notBefore, notAfter], for exercising the
// renewal timer arithmetic.
func datedRegistration(t *testing.T, nodeID string, notBefore, notAfter time.Time) *Registration {
	t.Helper()
	...
	cred, err := newRegistration(key, []*x509.Certificate{leaf}, "")
	if err != nil {
...
func (f *fakeRenewClient) setNext(cred *Registration) {
	f.mu.Lock()
	f.next = cred
	f.mu.Unlock()
}
...
func (f *fakeRenewClient) Renew(context.Context) (*Registration, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	f.renews++
	return f.next, nil
...
func (f *fakeRenewClient) SetRegistration(*Registration) {}
