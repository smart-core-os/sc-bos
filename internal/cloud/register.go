...
// in the subject, and returns the issued Credential (key + Connect-CA chain).
// The key never leaves the controller.
func Register(ctx context.Context, enrollmentCode, registerURL, nodeName string, opts ...RegisterOption) (*Credential, error) {
...
// it to the EST-shaped endpoint at url, and returns the issued Credential (key +
...
func enroll(ctx context.Context, httpClient *http.Client, url, commonName, bearer string) (*Credential, error) {
	...
	return newCredential(key, chain)
}

// postCSR submits a DER PKCS#10 CSR to an EST-shaped enrollment endpoint and
// returns the issued certificate chain (leaf first). The CSR is base64-encoded
// per application/pkcs10 + Content-Transfer-Encoding: base64; the response is a
// PEM chain (application/pem-certificate-chain). bearer, when non-empty, is sent
// as the Authorization (used by registration with the enrollment code; renewal
