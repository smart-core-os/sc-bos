...
func TestCredential_IntermediateChain(t *testing.T) {
	...
	cred, err := newCredential(leafKey, chain)
	if err != nil {
	...
	credStore, _ := newStores(t)
	if err := credStore.Save(context.Background(), cred); err != nil {
		t.Fatalf("save: %v", err)
	}
	reloaded, ok, err := credStore.Load(context.Background())
	if err != nil || !ok {
	...
	t.Run("intermediate omitted: check-in fails", func(t *testing.T) {
		...
		partial, err := newCredential(leafKey, []*x509.Certificate{chain[0], rootCert})
		if err != nil {
			t.Fatalf("newCredential: %v", err)
		}
