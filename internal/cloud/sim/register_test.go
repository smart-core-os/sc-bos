...
func TestDeviceRegister(t *testing.T) {
	...
	t.Run("registers with valid code and issues a cert with CN=nodeId", func(t *testing.T) {
		resp, chain := doRegister(t, client, ts.URL, freshCode(t), "test/path/AC-01")
		assertStatus(t, resp, http.StatusCreated)
	...
	t.Run("code is single-use", func(t *testing.T) {
		code := freshCode(t)
		resp, _ := doRegister(t, client, ts.URL, code, "test")
		assertStatus(t, resp, http.StatusCreated)
	...
	t.Run("unknown code is rejected", func(t *testing.T) {
		resp, _ := doRegister(t, client, ts.URL, "ZZZZZZ", "test")
		assertStatus(t, resp, http.StatusUnauthorized)
	})
...
// doRegister POSTs a CSR to the registration endpoint with code as the bearer token.
func doRegister(t *testing.T, client *http.Client, base, code, cn string) (*http.Response, []*x509.Certificate) {
	t.Helper()
	return doRegisterReq(t, client, deviceRegisterURL(base), code, cn)
}
