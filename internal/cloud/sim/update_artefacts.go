...
// toUpdateArtefact maps stored artefact metadata to its JSON representation. The payload is never part
// of the row; it is downloaded separately from PayloadURL. The sha256 is hex-encoded for the wire.
func toUpdateArtefact(r *http.Request, a store.UpdateArtefact) UpdateArtefact {
	out := UpdateArtefact{
		ID:         a.ID,
		Platform:   a.Platform,
		Version:    a.Version,
	...
	if len(a.Sha256) > 0 {
		out.SHA256 = hex.EncodeToString(a.Sha256)
	}
...
func toUpdateArtefacts(r *http.Request, as []store.UpdateArtefact) []UpdateArtefact {
...
func (s *Server) listUpdateArtefacts(w http.ResponseWriter, r *http.Request) {
	...
	items, err := s.store.ListUpdateArtefacts(r.Context(), queries.ListUpdateArtefactsParams{
		AfterID:  afterID,
		Platform: platform,
		SiteID:   siteID,
		Limit:    limit + 1,
...
func (s *Server) getUpdateArtefact(w http.ResponseWriter, r *http.Request) {
	...
	item, err := s.store.GetUpdateArtefact(r.Context(), id)
	if err != nil {
		resErr := translateDBError(err)
		writeError(w, resErr)
		if resErr.internal() {
