...
func credentialIDFromCert(cert *x509.Certificate) string {
	for _, u := range cert.URIs {
		if id, ok := strings.CutPrefix(u.String(), credentialURIPrefix); ok {
			return id
		}
	}
	return ""
...
// leafFingerprint returns the raw SHA-256 of a certificate's DER, used as a
// stable per-certificate identifier in the credential store.
