...
// CreateUpdateArtefact inserts a new update artefact, streaming src to exhaustion into an external
// payload file while computing its SHA-256 and recording the number of bytes read. The payload is
// never buffered in full. It returns the created row (including the generated id, the computed
// sha256, the size, and the create time) so the caller need not read it back.
//
...
func (s *Store) CreateUpdateArtefact(ctx context.Context, p CreateUpdateArtefactParams, src io.Reader) (queries.UpdateArtefact, error) {
	...
	if copyErr != nil {
		_ = tmp.Close()
		return queries.UpdateArtefact{}, fmt.Errorf("stream payload: %w", copyErr)
	}
	...
	var artefact queries.UpdateArtefact
	err = s.Write(ctx, func(tx *Tx) error {
		var err error
		artefact, err = tx.CreateUpdateArtefact(ctx, qp)
		if err != nil {
			return err
		}
		if err := os.Rename(tmpName, s.artefactPath(artefact.ID)); err != nil {
			return err
		}
		renamed = true
		return nil
