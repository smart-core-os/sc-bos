...
// testRegistration builds a self-signed credential whose leaf CN is nodeID.
func testRegistration(t *testing.T, nodeID string) *Registration {
	t.Helper()
...
func TestCredential_NodeIDAndTLS(t *testing.T) {
	cred := testRegistration(t, "node-123")
...
func TestFileRegistrationStore_RoundTrip(t *testing.T) {
	dir := t.TempDir()
	store := NewFileRegistrationStore(filepath.Join(dir, "registration.json"))
	...
	cred := testRegistration(t, "node-abc")
