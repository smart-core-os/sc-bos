...
func TestShouldDeleteInOwnedDir(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "plain pb.go with bos source header deleted",
			filename: "foo.pb.go",
			content:  "// source: smartcore/bos/foo/foo.proto\npackage foo\n",
			want:     true,
		},
		{
			name:     "grpc pb.go without bos source header preserved",
			filename: "foo_grpc.pb.go",
			content:  "// source: foo.proto\npackage foo\n",
			want:     false,
		},
		{
			name:     "grpc pb.go with bos source header deleted",
			filename: "foo_grpc.pb.go",
			content:  "// source: smartcore/bos/foo/foo.proto\npackage foo\n",
			want:     true,
		},
...
func TestCleanOwnedDir(t *testing.T) {
	...
	files := map[string]string{
		"count.pb.go":        "// source: smartcore/bos/count/v1/count.proto\npackage countpb\n",
