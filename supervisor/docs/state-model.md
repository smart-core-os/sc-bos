...
recreated from `:current` on restart. FreeBSD has no container tags, but the shape is the same: an
applied version, a recorded previous version, the version actually running, and a restart that brings up
...
1. **Download and verify**: update artefact is downloaded to disk and its hash verified. 
...
The durable state is a JSON file at `<StateDir>/state.json`: the goal (target version, URL, hash), the last committed 
