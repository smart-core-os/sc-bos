#!/usr/bin/env bash
# 02-cloudsim.sh (run on the Mac) — start cloudsim as the update server and provision the test node.
#
# Starts cloudsim bound to all interfaces (so the machine can reach it), creates a site + node, runs
# the real /v1/device/register exchange to obtain valid client credentials, uploads the v2 + v2bad
# artefacts, and records everything 03-install.sh / 04-deploy.sh need in $STATE_ENV.
#
...
NODE_ID=$(curl -fsS -X POST "$MGMT/nodes" -H 'Content-Type: application/json' \
  -d "{\"hostname\":\"$MACHINE\",\"siteId\":\"$SITE_ID\",\"platform\":\"$PLATFORM\"}" | jq -r '.id')
...
# Register from the Mac via loopback (203.0.113.113 is only routable from inside the machine). We do
# not use the response's host-derived bosapi_root — 03-install.sh writes the machine-reachable
# $BOSAPI_ROOT into registration.json. post: CLIENT_ID/CLIENT_SECRET are valid node credentials.
...
# post: $STATE_ENV is a sourceable file (valid on both Mac and machine) with all ids/credentials.
cat >"$STATE_ENV" <<EOF
