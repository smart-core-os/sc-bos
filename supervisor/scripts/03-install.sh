...
# the host Supervisor (which performs the download) reach. post: <data>/cloud/registration.json
# (mode 0600) carries the credentials from step 02; BOS loads it on startup and moves to the
# Connecting state, then polls /v1/device/check-in via bosapi_root.
