#!/usr/bin/env bash
# 02-cloudsim.sh — start cloudsim as the binary server and provision the demo node.
#
...
# post: NODE_ID names a node in that site (its initial secret is unused — we register below). The
#       node's os/arch are also reconciled from what BOS reports on check-in.
NODE_ID=$(curl -fsS -X POST "$MGMT/nodes" -H 'Content-Type: application/json' \
  -d "{\"hostname\":\"$NODE_HOSTNAME\",\"siteId\":\"$SITE_ID\",\"os\":\"$OS\",\"arch\":\"$ARCH\"}" | jq -r '.id')
...
# --- 4. upload the two binary artefacts -----------------------------------------------------------
...
upload() {  # upload VERSION TARBALL -> prints artefact id
  curl -fsS -X POST "$MGMT/binary-artefacts" \
    -F "version=$1" -F "os=$OS" -F "arch=$ARCH" -F "siteId=$SITE_ID" -F "payload=@$2" | jq -r '.id'
}
...
cat >"$STATE_ENV" <<EOF
# Generated by 02-cloudsim.sh — runtime state for the supervisor binary demo.
SITE_ID="$SITE_ID"
NODE_ID="$NODE_ID"
CLIENT_ID="$CLIENT_ID"
