#!/usr/bin/env bash
# 02-cloudsim.sh — start cloudsim as the update server and provision the demo node.
#
...
cat >"$STATE_ENV" <<EOF
# Generated by 02-cloudsim.sh — runtime state for the supervisor update demo.
SITE_ID="$SITE_ID"
NODE_ID="$NODE_ID"
CLIENT_ID="$CLIENT_ID"
