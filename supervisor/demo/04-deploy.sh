#!/usr/bin/env bash
# 04-deploy.sh <v1|v2|v2bad> — create a binary deployment and watch it settle.
#
...
# install runs device-side: BOS picks up latestBinary on its next ~30s check-in, calls the Supervisor,
...
DEP_ID=$(curl -fsS -X POST "$MGMT/binary-deployments" -H 'Content-Type: application/json' \
  -d "{\"binaryArtefactId\":\"$ART_ID\",\"nodeId\":\"$NODE_ID\"}" | jq -r '.id')
...
while (( SECONDS < deadline )); do
  body=$(curl -fsS "$MGMT/binary-deployments/$DEP_ID")
