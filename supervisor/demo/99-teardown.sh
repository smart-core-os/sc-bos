...
# post: installed binary/config + durable state + demo users file + the named BOS data volume are removed.
sudo rm -rf "$SUP_INSTALL" "$SUP_CONF_DIR" "$SUP_STATE_DIR" "$BOS_USERS_FILE"
