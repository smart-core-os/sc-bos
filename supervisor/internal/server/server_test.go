...
func TestInstallUpdate_EchoesDeploymentID(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		if _, err := c.InstallUpdate(context.Background(), &supervisorpb.InstallUpdateRequest{
			Version: "v2", DownloadUrl: url, Sha256: sum, DeploymentId: "42",
		}); err != nil {
			t.Fatal(err)
		}
		synctest.Wait()
...
func TestInstallUpdate_FailedDownload(t *testing.T) {
	t.Run("http error", func(t *testing.T) {
		...
		synctest.Test(t, func(t *testing.T) {
			mustInstall(t, c, "v2", srv.URL, hex.EncodeToString(make([]byte, sha256.Size)))
			synctest.Wait()
	...
	t.Run("checksum mismatch", func(t *testing.T) {
		...
		synctest.Test(t, func(t *testing.T) {
			mustInstall(t, c, "v2", url, hex.EncodeToString(make([]byte, sha256.Size)))
			synctest.Wait()
